...
func NewHTTPSearchTool(cfg *common.Config) tool.InvokableTool {
	...
	if cfg.Services.MCPCommand != "" {
		...
		t, _ := utils.InferOptionableTool("http_search", "通过 MCP(go-sdk) 调用外部检索服务", func(ctx context.Context, in *HTTPSearchInput, opts ...tool.Option) (string, error) {
			out, err := gosdk.InvokeUsingCommand(ctx, cmd, []string{}, "search", map[string]any{"query": in.Query})
			if err != nil {
				return "检索失败", nil
			}
			return out, nil
